...
/**
 ...
 spark-submit --class cn.datalake.streaming.Kafka2Spark2HDFS \
 --master yarn \
 --deploy-mode client \
 --executor-memory 2g \
 --executor-cores 2 \
 ...
 --num-executors 2 \
 --queue default  \
 --principal svchadoop@LTS.LOCAL \
 --keytab /home/svchadoop/.svchadoop.keytab \
 --driver-java-options "-Djava.security.auth.login.config=/usr/hdp/current/kafka-broker/config/kafka_client_jaas.conf" \
 --conf "spark.executor.extraJavaOptions=-Djava.security.auth.login.config=/usr/hdp/current/kafka-broker/config/kafka_client_jaas.conf" \
 cn-datalake-sample-1.0-SNAPSHOT.jar
  */
object Kafka2Spark2HDFS {

  Logger.getLogger("com").setLevel(Level.ERROR) //设置日志级别

  var confPath: String = System.getProperty("user.dir") + File.separator + "conf/app.properties"
  ...
  def main(args: Array[String]): Unit = {
    ...
    if(!file.exists()) {
      System.out.println(Kafka2Spark2Hive.getClass.getClassLoader.getResource("app.properties"))
      val in = Kafka2Spark2Hive.getClass.getClassLoader.getResourceAsStream("app.properties")
      properties.load(in);
    } else {
    ...
    val topics = properties.getProperty("topic.name")
